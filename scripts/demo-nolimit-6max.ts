/**
 * E2E Demo: 6 bots play 100 hands of No-Limit Hold'em.
 ...
 *
 * Usage: npx tsx scripts/demo-nolimit-6max.ts
...
// ShortStackBot: push-or-fold with top 25%
function shortStackAction(
  legal: ActionType[],
  state: GameState,
  playerId: string
): { type: ActionType; amount?: number } {
  ...
  if (legal.includes(ActionType.RAISE)) {
    return { type: ActionType.RAISE, amount: Math.min(allInAmount, ranges.maxRaise) };
  }

  if (legal.includes(ActionType.BET)) {
    return { type: ActionType.BET, amount: Math.min(allInAmount, ranges.maxBet) };
  }
...
const BOTS = [
  ...
  { id: 'ShortStack', strategy: 'short-stack' },
...
async function main() {
    ...
    while (!state.isHandComplete) {
      ...
      switch (bot.strategy) {
        ...
        case 'pot-control':
          ...
          break;
        case 'short-stack':
          action = shortStackAction(legal, state, activePlayer.id);
          break;
        default:
  ...
  const r_chips: number[] = BOTS.map(() => STARTING_CHIPS);
  let r_dealer = 0;

  for (let h = 0; h < records.length && h < 10; h++) {
    // Test first 10 hands
    const playersWithChips = BOTS.map((b, i) => ({ ...b, seatIndex: i, chips: r_chips[i]! })).filter(
      (b) => b.chips > 0
    );
    ...
    while (!state.isHandComplete) {
      ...
      switch (bot.strategy) {
        ...
        case 'short-stack':
          ...
          break;
        default:
          action = randomAction(legal, state, activePlayer.id, botRng2);
      }
    ...
    const match = BOTS.every((_, i) => r_chips[i] === original.chipsAfter[i]);
    if (!match) {
      console.error(
        `  [FAIL] Replay mismatch at hand ${h + 1}: orig=[${original.chipsAfter}] replay=[${r_chips}]`
      );
      replayOk = false;
    }
  }
  console.log(`  Replay determinism (first 10 hands): ${replayOk ? 'PASS' : 'FAIL'}`);
  ...
  for (const s of stats) {
    ...
    console.log(
      `  ${s.id.padEnd(16)} | Hands: ${s.handsPlayed.toString().padStart(3)} | VPIP: ${vpipPct.padStart(5)}% | PFR: ${pfrPct.padStart(5)}% | Win: ${winRate.padStart(5)}% | P/L: ${profit >= 0 ? '+' : ''}${profit}`
    );
  }
  ...
  const allPass =
    chipConservationOk && globalChipOk && chainOk && replayOk && records.length >= NUM_HANDS;
  console.log();
